...
bool packToken::operator==(const packToken& token) const {
  if (tokType_NUM & token.base->type & base->type) {
...
std::string& packToken::asString() const {
  if (base->type != tokType_STR && base->type != tokType_VAR && base->type != tokType_OP) {
    throw bad_cast(
